...
const formatErrorMessage = (errorMessage: string): string => {
  ...
  // User-friendly messages for common API/network issues
  if (lower.includes('fetch') && (lower.includes('failed') || lower.includes('not available') || lower.includes('unavailable'))) {
    return 'Service temporarily unavailable. Please try again in a moment.';
  }
  if (lower.includes('network') || lower.includes('econnrefused') || lower.includes('etimedout') || lower.includes('enotfound')) {
    return 'Connection issue. Please check your network and try again.';
  }
  if (lower.includes('not convertable') || lower.includes('not supported') || lower.includes('not available for exchange')) {
    return 'This currency pair is not available for exchange. Please try a different pair (e.g. BTC, ETH, USDT).';
  }
...
export default function ExchangeWidget() {
  ...
  // Fetch prices for all enabled cryptos so the selector dropdown shows live prices for every coin
  ...
  // Fee % from backend (defaults while loading)
  ...
  // Sanity bounds for exchange rate (1 unit send = rate units receive). Prevents glitches from wrong/stale prices.
  const SANE_RATE_MIN = 1e-12;
  const SANE_RATE_MAX = 1e12;

  // Use locked rate for fixed, live rate for float. Never expose an out-of-range rate (prevents display glitches).
  const exchangeRate = useMemo(() => {
    const raw = orderType === "fixed" ? (lockedExchangeRate ?? liveExchangeRate) : liveExchangeRate;
    if (raw === null || !Number.isFinite(raw)) return null;
    if (raw < SANE_RATE_MIN || raw > SANE_RATE_MAX) return null;
    return raw;
  ...
  useEffect(() => {
    ...
    // When pair changes, clear rates immediately so we never show the previous pair's rate
    if (pairChanged) {
      setLiveExchangeRate(null);
    ...
    // Rate = how many units of receive per 1 unit of send. Must be sendPrice/receivePrice (not inverted).
    const rate = sendPrice / receivePrice;

    if (rate < SANE_RATE_MIN || rate > SANE_RATE_MAX || !Number.isFinite(rate)) {
      setLiveExchangeRate(null);
      return;
    }
  ...
  // Fetch exchange fee % from backend (single load)
  useEffect(() => {
    let cancelled = false;
  ...
  // Min/max validation is done only on "Exchange Now" click — we do not show or block by limits in the UI until then.
  ...
  // True when the provider (NOWPayments) does not support this pair — show message and disable Exchange
  const isUnsupportedPair = Boolean(
    limitsError && limitsError.includes('not available for exchange')
  );
      ...
      <div className="glass-panel rounded-xl sm:rounded-2xl shadow-2xl shadow-black/50 overflow-visible relative z-10">
        ...
        <div className="flex border-b border-white/5">
          <button
            ...
            className={`flex-1 py-3 sm:py-4 text-xs sm:text-sm font-medium transition-colors border-b-2 ${orderType === "fixed"
          ...
          <button
            ...
            className={`flex-1 py-3 sm:py-4 text-xs sm:text-sm font-medium transition-colors border-b-2 ${orderType === "float"
                ? "text-white border-blue-500 bg-white/5"
                : "text-neutral-400 border-transparent hover:text-white"
              }`}
          >
        ...
        <div className="p-4 sm:p-5 md:p-6 space-y-3 sm:space-y-4">
          ...
          <div className="space-y-1.5 sm:space-y-2">
            <div className="flex justify-between text-[10px] sm:text-xs text-neutral-400">
              <span>You Send</span>
              {/* Min/max are not shown here — user sees minimum only when they click Exchange and amount is below min */}
            </div>
            ...
            {/* Amount errors (e.g. below minimum) are shown only after user clicks Exchange Now, in the order error banner below */}
            <div className="min-h-[12px] px-2" aria-hidden="true" />
          </div>
          ...
          <div className="space-y-1.5 sm:space-y-2">
            ...
            <div className="input-group rounded-lg sm:rounded-xl flex items-center p-1 relative">
              <input
                ...
                value={receiveAmount}
                readOnly
                className={`w-full bg-transparent border-none text-white text-lg sm:text-xl md:text-2xl font-medium px-2 sm:px-3 md:px-4 focus:ring-0 placeholder-neutral-600 outline-none cursor-default transition-opacity duration-200 ${exchangeRate === null && pricesLoading ? 'opacity-60' : 'opacity-100'
                  }`}
              />
          ...
          {/* Unsupported pair — provider doesn't support this combination */}
          {isUnsupportedPair && (
            <div className="mb-4 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3">
              <p className="text-sm font-medium text-amber-200">
                This pair isn&apos;t available for exchange
              </p>
              <p className="mt-1 text-xs text-amber-200/80">
                Our provider supports main pairs like BTC, ETH, USDT, LTC, and others. Try switching send or receive currency to a supported pair.
              </p>
            </div>
          )}
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (!limitsResponse.ok) {
                  setIsCreatingOrder(false);
                  const errData = await limitsResponse.json().catch(() => ({}));
                  setOrderError(formatErrorMessage(errData.error || 'Unable to validate limits. Please try again.'));
                  return;
                }
                ...
                if (!limitsData.success || !limitsData.limits) {
                  setIsCreatingOrder(false);
                  setOrderError(formatErrorMessage(limitsData.error || 'Unable to validate limits. Please try again.'));
                  return;
                }

                const freshLimits = limitsData.limits;
                const freshEffectiveMin = freshLimits.min_amount * (1 + MIN_AMOUNT_BUFFER);
                const pricePerUnit = prices[sendCoingeckoId]?.usd ?? 0;
                const minUsd = freshEffectiveMin * pricePerUnit;

                if (amount < freshEffectiveMin) {
                  setIsCreatingOrder(false);
                  const minMessage = minUsd > 0
                    ? `Minimum swap amount is $${minUsd.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}.`
                    : `Minimum swap amount is ${formatMinAmount(freshEffectiveMin)} ${displaySendCrypto?.symbol || ''}.`;
                  setOrderError(minMessage);
                  return;
                }

                if (freshLimits.max_amount && amount > freshLimits.max_amount) {
                  setIsCreatingOrder(false);
                  setOrderError(`Amount exceeds maximum. Maximum swap amount is ${formatMinAmount(freshLimits.max_amount)} ${displaySendCrypto?.symbol || ''}.`);
                  return;
                }
