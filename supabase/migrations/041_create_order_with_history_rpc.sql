-- Transactional order creation: order + initial status history in one atomic operation.
-- Prevents state where payment exists in NOWPayments but no order in our DB.
...
COMMENT ON FUNCTION create_order_with_history(jsonb) IS 'Creates order and initial status history in one transaction. Use from payment route to avoid orphan payments.';
